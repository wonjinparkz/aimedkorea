...
# AI-MED Korea Android 키스토어 생성 스크립트

echo "🔐 AI-MED Korea Android 키스토어 생성"
echo "========================================="
echo ""
...
echo ""
read -p "조직명 (Organization): " ORG_NAME
read -p "조직 단위 (Organizational Unit): " ORG_UNIT
read -p "도시 (City): " CITY
read -p "지역/주 (State/Province): " STATE
read -p "국가 코드 (Country Code, 예: KR): " COUNTRY

# 키스토어 비밀번호 입력
echo ""
echo "⚠️  비밀번호는 최소 6자 이상이어야 합니다."
read -sp "키스토어 비밀번호: " KEYSTORE_PASS
echo ""
read -sp "키스토어 비밀번호 확인: " KEYSTORE_PASS_CONFIRM
echo ""
...
- ✅ .gitignore에 android-signing/ 디렉토리 추가 필요
...
EOL
    ...
    echo ""
    echo "⚠️  중요: android-signing 디렉토리를 .gitignore에 추가하세요!"
    echo ""
else
    echo "❌ 키스토어 생성 실패"
    exit 1
